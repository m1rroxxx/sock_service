...
@Component
public class LessThanCottonPercentageFilter implements Filter<Sock, SockFilterDto>  {
    @Override
    public boolean isApplicable(SockFilterDto filter) {
        return filter.getLessThanCottonPercentage() != null;
    }

    @Override
    public Stream<Sock> apply(SockFilterDto filter, Stream<Sock> stream) {
        return stream.filter(sock -> sock.getCottonPercentage() < filter.getLessThanCottonPercentage());
    }
}
